...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'markov':
        ...
        print(generate_toyLM_ngram_a(table, context='aleatorio', n=sys.argv[2])) 
        print(perplexity_ngrams(texts2ids_test))
        ...
        model = Sequential([Embedding(5616, 20), LSTM(64), Dense(5616, activation='softmax')])

        model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])

        history = model.fit(x_train, y_train, batch_size=64, epochs=15)

        print(generate_toyLM_lstm_a(model, context='aleatorio', n=sys.argv[2]))
